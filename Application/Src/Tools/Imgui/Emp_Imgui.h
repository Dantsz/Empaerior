...
static void AttachImgui(Empaerior::Window &window, VK_Renderer &renderer)
{
    ...
    VkCommandBuffer command_buffer =
        Empaerior::VKfunctions::beginSingleTimeCommands(renderer.device, renderer.graphicsQueue, renderer.commandPool);
    ImGui_ImplVulkan_CreateFontsTexture(command_buffer);
    Empaerior::VKfunctions::endSingleTimeCommands(renderer.device, renderer.graphicsQueue, renderer.commandPool,
                                                  command_buffer);
}
...
EMP_FORCEINLINE void refreshImgui(Empaerior::Window &window, VK_Renderer &renderer)
{
    ...
    if (renderer.framebufferNeedsReconstruction)
    {
        ...
        //	ImGui_ImplVulkanH_CreateOrResizeWindow(renderer.instance, renderer.physicalDevice, renderer.device, &wd,
        //renderer.findQueueFamilies(render).graphicsFamily.value(), nullptr, window.get_width(), window.get_heigth(),
