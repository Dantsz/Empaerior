...
struct Texture_Atlas {
    ...
    size_t create_texture_from_file(const std::string& path, bool& implementUpdate)
    {
        ...
        image_dimensions.push_back({ static_cast<Empaerior::fl_point>(texWidth),static_cast<Empaerior::fl_point>(texHeight) });
        ...
        createImage(*m_allocator, texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, images[images.size() - 1], image_allocations[image_allocations.size() - 1]);
        ...
        copyBufferToImage(*m_device, *m_graphicsqueue, *m_commandpool, stagingBuffer, images[images.size() - 1], static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
