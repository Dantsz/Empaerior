...
struct DynamicBuffer
{
    ...
    void Init(size_t initialSize)
    {
        for (size_t i = 0; i < buffering; i++)
        {
            ...
            Empaerior::VKfunctions::allocateBuffer(*m_allocator, BufferSize[inUseBufferIndex] + initialSize, usage,
                                                   &Buffers[i], &BuffersAllocations[i], &bufferAllocInfo);
    ...
    void ExpandBuffer(size_t size)
    {
        ...
        Empaerior::VKfunctions::allocateBuffer(*m_allocator, BufferSize[srcdataIndex] + size, usage, &stagingBuffer,
                                               &stagingBufferAllocation, &bufferAllocateInfo);
...
{
    uint32_t *index_data =
        static_cast<uint32_t *>(geometrybuffer.indexBuffer.BuffersData[geometrybuffer.indexBuffer.get_in_use_index()]);
    std::cout << "======Index=====\n";
    std::cout << geometrybuffer.indexBuffer.used_size[geometrybuffer.indexBuffer.get_in_use_index()] / sizeof(uint32_t)
              << " will be drawn\n";
    for (size_t i = 0;
         i < (geometrybuffer.indexBuffer.used_size[geometrybuffer.indexBuffer.get_in_use_index()] / sizeof(uint32_t));
         i++)
    {
...
EMP_FORCEINLINE void dump_VertexData(geometryBuffer &buffer)
{
    ...
    for (size_t i = 0; i < buffer.vertexBuffer.used_size[buffer.indexBuffer.get_in_use_index()] / sizeof(Vertex); i++)
    {
        if (i % 4 == 0)
            std::cout << i << ":\n";
        std::cout << vertex_data[i].pos.x << ' ' << vertex_data[i].pos.y << ' ' << vertex_data[i].pos.z << ' '
                  << vertex_data[i].texCoord.x << ' ' << vertex_data[i].texCoord.y << ' ' << vertex_data[i].tex_id
                  << '\n';
    }
}
